...
namespace FilmrecAPI.bzl
{
    public class RecommenderBzl : IRecommenderBzl
    {
        ...
        private async Task<List<RecMedia>> getMoviesByUserPicks(RecommenderContext recommenderContext)
        {
            ...
            {
                string urlString = string.Format(baseUrl + "/discover/movie?api_key={0}&language={1}&with_people={2}&with_genres={3}&with_runtime.gte={4}&with_runtime.lte={5}&sort_by=popularity.desc&vote_count.gte=100&page={6}",
        ...
        private async Task<List<RecMedia>> getTVbyGenre(RecommenderContext recommenderContext)
        {
            ...
            for (int page = 1; page <= maxNrPages; page++)
            {
                string urlString = string.Format(baseUrl + "/discover/tv?api_key={0}&language={1}&with_genres={2}&sort_by=popularity.desc&vote_count.gte=100&page={3}",
